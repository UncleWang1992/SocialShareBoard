...
#import <pop/POP.h>
#import "ShareImageWithLabel.h"

#define ScreenWidth         [[UIScreen mainScreen] bounds].size.width
#define ScreenHeight        [[UIScreen mainScreen] bounds].size.height
#define WINDOW_COLOR        [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4]
#define TOPSHEETVIEW_COLOR         [UIColor colorWithRed:243/255.0 green:244/255.0 blue:244/255.0 alpha:1]
#define ANIMATE_DURATION    0.25f
#define LINE_HEIGHT         104
...
- (void)loadUIConfig {
    ...
    for (NSInteger i = 0; i < self.shareBtnTitleArray.count; i++) {
        ...
        ShareImageWithLabel *item = [ShareImageWithLabel imageLabelWithFrame:frame Image:[UIImage imageNamed:self.shareBtnImageArray[i]] LabelText:self.shareBtnTitleArray[i]];
        item.labelOffsetY = 6;
        item.tag = 200 + i;
...
- (UIView *)bgView {
    if (_bgView == nil) {
        ...
        _bgView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, BUTTON_HEIGHT + (_protext.length == 0 ? 0 : 45) + LINE_HEIGHT * index + [self getWindowSafeAreaBottom]);
        _bgView.backgroundColor = TOPSHEETVIEW_COLOR;
    }
    return _bgView;
}
...
- (UIView *)topSheetView {
    if (_topSheetView == nil) {
        _topSheetView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_bgView.frame), CGRectGetHeight(_bgView.frame) - BUTTON_HEIGHT  + [self getWindowSafeAreaBottom])];
        _topSheetView.backgroundColor = TOPSHEETVIEW_COLOR;
        //    如果有标题，添加标题
        if (_protext.length) {
...
- (UIButton *)cancelBtn {
    if (_cancelBtn == nil) {
        ...
        _cancelBtn.frame = CGRectMake(30, CGRectGetHeight(_bgView.frame) - BUTTON_HEIGHT- [self getWindowSafeAreaBottom], CGRectGetWidth(_bgView.frame) - 30*2, BUTTON_HEIGHT);
        //    取消按钮
...
- (CGFloat)getWindowSafeAreaBottom{
    if (@available(iOS 11.0, *)) {
        CGFloat safeAreaBottom = [UIApplication sharedApplication].delegate.window.safeAreaInsets.bottom;//34
